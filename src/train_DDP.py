...
def allReduce(data):
    ...
    '''
    function:integrate data from multiple processes
    input: int or float
    output: float
    '''
    
    data = torch.tensor(float(data)).cuda()
    dist.all_reduce(data, op=dist.ReduceOp.SUM, group=dist.group.WORLD, async_op=False)
    data /= torch.cuda.device_count()
    data = data.item()
...
print('---------------------------------------- step 1/5 : parameters preparing... ----------------------------------------') if opt.local_rank==0 else print('',end='')
print(opt) if opt.local_rank==0 else print('',end='')
...
print('---------------------------------------- step 2/5 : data loading... ------------------------------------------------') if opt.local_rank==0 else print('',end='')
dataset = datasets.MNIST(opt.dataset, train=True, download=True,
        transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize([0.5], [0.5])]))

# new #
sampler = distributed.DistributedSampler(dataset)
dataloader = DataLoader(dataset=dataset, batch_size=opt.batch_size, num_workers=opt.workers, sampler=sampler)

time_2 = time.time()

print('---------------------------------------- step 3/5 : model defining... ----------------------------------------------') if opt.local_rank==0 else print('',end='')
# new #
...
print('---------------------------------------- step 4/5 : requisites defining... -----------------------------------------') if opt.local_rank==0 else print('',end='')
# Loss function
...
for epoch in range(opt.epochs):
    ...
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        # new #
        dist.barrier()
        loss = allReduce(loss.item())
        acc = allReduce(acc)
        
        print('Training: Epoch[{:0>3}/{:0>3}] Iteration[{:0>4}/{:0>4}] Loss: {:.4f} Acc: {:.4f}'.format(
                       epoch + 1, opt.epochs, i + 1, len(dataloader), loss, acc)) if opt.local_rank==0 else print('',end='')
